export const LoginInputs = ({ label, placeholder, type, bg, register, name, value, onChange}) => {
    return (
        <div className="text-sm w-full">
            <label className="text-gray-200 font-semibold">{label}</label>
            <input
                required
                name={name}
                value={value}
                onChange={onChange}
                {...register}
                type={type} 
                placeholder={placeholder} 
                className={`w-full text-sm mt-2 p-5 border border-border rounded text-white ${ bg ? 'bg-main' : "bg-dry"}`}
            />
        </div>
    )
};

export const Message = ({ label, placeholder, register, name }) => {
    return (
        <div className="text-sm w-full">
            <label className="text-border">{label}</label>
            <textarea 
                className="w-full h-40 mt-2 p-6 bg-main border border-border rounded" 
                placeholder={placeholder}
                name={name}
                {...register}
                ></textarea>
        </div>
    );
};

export const Select = ({ label, options, onChange, register, name }) => {
    return (
        <>
        <label className="text-border font-semibold">{label}</label>
        <select 
            name={name}
            {...register}
            className="w-full mt-2 px-6 py-4 text-text bg-main border border-border" 
            onChange={onChange}>
            {
                options.map((o, i) => (
                    <option key={i} value={o.value}>
                        {o.title}
                    </option>
                ))}
        </select>
        </>
    );
};