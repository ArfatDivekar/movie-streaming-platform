...
const Navbar = () => {
  ...
  const { likedMovies } = useSelector((state) => state.userGetFavouriteMovies);
  ...
  return (
    <div>
      {/* <div className="bg-main top-0 sticky z-20">
      ...
      <div className="w-full top-0 left-0">
        <div className="md:flex items-center justify-between bg-main py-4 md:px-10 px-7">
          ...
          {/* NAVLINKS SECTION  */}
          <ul
            className={`md:flex md:items-center md:justify-center md:pb-0 pb-12 absolute md:static bg-main md:z-auto z-10 left-0 w-full md:w-auto md:pl-0 pl-9 transition-all ease-in ${
              open ? "top-30 opacity-100" : "top-[-490px]"
            } opacity-0 md:opacity-100`}
          >
            {/* {
                    NavLinks.map((link) => (
                        <li key={link.name} className="md:ml-8 text-white font-medium text-sm md:my-0 my-7">
                            <a href={link.link}>{link.name}</a>
                        </li>
                    ))
                } */}
            <li className="md:ml-14 text-white font-medium text-sm md:my-0 my-7">
              <NavLink to="/movies" className={Hover}>
                Movies
              </NavLink>
            </li>
