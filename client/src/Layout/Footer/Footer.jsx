      ...
      <div className="container mx-auto px-2">
        <div className="grid grid-cols-2 md:grid-cols-7 xl:grid-cols-12 gap-5 sm:gap-9 lg:gap-11 xl:gap-7 justify-between">
              ...
              <ul className='text-sm flex flex-col space-y-3'>
                {link.links.map((link, index) => (
                  <li key={index} className='flex items-baseline'>
                    <Link to={link.link} target={link.link.startsWith("http") ? "_blank" : ""} className='text-slate-400 inline-block w-full hover:text-subMain transition'>
                      {link.name}
                    </Link>
                  </li>
                ))}
          ...
          <div className="pb-3.5 sm:col-span-0 md:col-span-2 lg:col-span-3 ">
              <Link to='/'>
                  <img src={Logo} alt="logo" className='w-2/4 h-12 object-contain' />
              </Link>
              <p className="leading-7 text-sm mt-3 text-slate-400 ">
                  <span>
                    Gowalkot Road<br /> Chiplun
                  </span>
                  <br />
                  <span>Tel: 8856900914</span>
                  <br />
                  <span>Email: arfatdivekar99@gmail.com</span>
              </p>
          </div>
        </div>
      </div>
